...
| **Checks: DoS** | GQL-007 depth, 008 complexity, 009 batch, **D01 alias amplification, D02 field duplication, D03 circular fragments, D04 directive overloading, D05 unbounded pagination** | A− | Depth uses latency heuristics (4×, ≥500ms) — noisy and evadable. GQL-D01–D05 now add structural alias-amplification, field-duplication, fragment-cycle, directive-overloading, and schema-driven pagination-abuse signals. Remaining gaps are the cost oracle, persisted queries, and introspection-as-DoS (GQL-D06–D08). |
